...
func Test_UpdateMovesPosInDirectionOfFirstVector(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			exp:  Position{x: 40, y: 41},
		},
		{
			name: "up",
			m:    vector{dir: up, mag: 3},
			exp:  Position{x: 40, y: 39},
		},
		{
			name: "right",
			m:    vector{dir: right, mag: 3},
			exp:  Position{x: 41, y: 40},
		},
		{
			name: "left",
			m:    vector{dir: left, mag: 3},
			exp:  Position{x: 39, y: 40},
		},
	}
...
func Test_MoveDoesNotChangeMagnitudeWhenOnlyOneSegmentExists(t *testing.T) {
	sn := newSnake(Position{x: 10, y: 10})
	sn.move(&board{
		upperLeft:  Position{x: 0, y: 0},
...
func Test_SnakeWontMoveOutsideOfThe(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Len(t, s.vecs, 1)
			require.Equal(t, Position{10, 10}, s.start, "starting Position shouldn't have changed")
		})
	}
}
...
func Test_SnakeGrowsByEatingApples(t *testing.T) {
	s := newSnake(Position{x: 10, y: 10})
	as := apples{
		{pos: Position{x: 11, y: 10}, eaten: false},
	}
	s.eat(as)
...
func Test_SnakeMovesFourSquaresPerSecond(t *testing.T) {
	...
	ticks := 0
	for range ticker.C {
		s.move(&board{upperLeft: Position{x: 0, y: 0}, lowerRight: Position{x: 20, y: 20}}, rate)
		ticks += 1
		if ticks == 20 {
			ticker.Stop()
			break
	...
	require.Equal(t, Position{x: 14, y: 10}, s.start, "starting Position should have moved 4 squares right")
}

func Test_NewSnakeState(t *testing.T) {
	scn := setupDefaultScreen(t)

	sn := newSnake(Position{x: 40, y: 40})
	sn.draw(scn)

	require.Equal(t, Position{x: 40, y: 40}, sn.start)
